...
export function useReducer(reducer, initialState) {
    ...
    function dispatch(action) {
        ...
        if (reducer) {
            let newState = reducer(hookStates[currentIndex], action)
            hookStates[currentIndex] = newState;
        } else {
...
/**
 * 
 * @param {*} parentDOM 老的父的真实dom
 * @param {*} oldVdom   老虚拟dom
 * @param {*} newVdom   新虚拟dom
 * @param {*} nextDOM  新虚拟dom下一个真实dom
 * @returns 
 */
...
function updateElement(oldVdom, newVdom) {
    if (oldVdom.type && oldVdom.type.$$typeof === REACT_MEMO) {
        ...
        // updateForwardRef(oldVdom, newVdom)
...
// 更新updateForwardRef
// function updateForwardRef(oldVdom, newVdom) {
//    //todo
// }
//更新memo
function updateMemoComponent(oldVdom, newVdom) {
    ...
    if (!type.compare(oldVdom.props, newVdom.props)) {
        ...
        let newRenderVdom = type.type(props);
        compareTwoVdom(parentDOM, oldVdom.oldRenderVdom, newRenderVdom);
        newVdom.oldRenderVdom = newRenderVdom;
    } else {
